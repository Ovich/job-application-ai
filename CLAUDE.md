...
- Cost posture: idle infrastructure under CHF 10 a month. Lambda outside any VPC, Aurora Serverless v2 paused when idle on a public endpoint with TLS forced and IAM database authentication, function URL behind CloudFront, SQS for jobs. The AI tokens are the only cost that scales, and the ledger passes them on.
...
17. **MUST NOT add a fixed monthly cost line without a decision recorded in the foundation first.** No Lambda inside a VPC (it forces a NAT gateway, about CHF 35 a month), no RDS Proxy, no API Gateway in front of the API, no always-on database capacity. The database is Aurora Serverless v2 with automatic pause, reached from outside the VPC over a public endpoint with `rds.force_ssl` on and an identity token minted per connection; the API is a function URL behind CloudFront; jobs go through SQS. The Data API is **not** used: it is absent from Zurich entirely and declined in Frankfurt, a pooled connection beating an HTTPS round trip per statement (board `D5`, 2026-09-08). The posture is `foundation.reference.md` F15: idle infrastructure under CHF 10 a month, AI tokens the only cost that scales. https://docs.aws.amazon.com/cdk/v2/guide/best-practices.html
